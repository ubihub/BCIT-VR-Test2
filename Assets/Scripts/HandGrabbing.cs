...
/* 
 * copied from a tech site 2020.06.13
 */
...
public class HandGrabbing : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //if don't have an active object in hand, look if there is one in proximity
        if (_currentGrabObject == null)
        {
            ...
            {
                //if there are colliders, take the first one if press the grab button and it has the tag for grabbing
                if (Input.GetAxis(InputName) >= 0.01f && colliders[0].transform.CompareTag(GrabTag))
                {
                    //if are already grabbing, return
                    if (_isGrabbing)
                    {
                        return;
                    }
                    ...
                    //set current object to the object have picked up (set it as child)
        ...
        //have object in hand, update its position with the current hand position (+defined offset from it)
        {

            //release grab button, release current object
